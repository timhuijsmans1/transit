...
def star_array(radius, matrix_size):
...
def star_mask_by_planet(star_array, cx, cy, r_planet, star_brightness):
    ...
    cy_projection = len(star_array) - 1 - cy
    matrix_size = len(r_planet)

    # set the part of the matrix to be replaced
    ymin = cy_projection - matrix_size / 2
    ymax = cy_projection + matrix_size / 2
...
if __name__ == "__main__":
    ...
    # create the star
    star = star_array(star_radius, star_array_size)
    ...
    scaled_kepler = kepler_coordinates * (star_radius / (2 * (10 ** -7)))
    translated_kepler = [x+(star_array_size / 2) for x in scaled_kepler]
    print(scaled_kepler)
    print(translated_kepler)
    print(star_array_size)
